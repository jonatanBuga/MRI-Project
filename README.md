...
## Next Steps (roadmap)
- Build metadata utilities (`scripts/update_metadata.py`) and implement mask validation routines.
- Implement a Dataset/DataLoader that splits data by subject and timepoint for proper cross-validation.
- Add robust MRI Z-score intensity normalization (scan- or subject-level) and test effects on performance.
- Integrate TransUNet (or other architectures) and begin fine-tuning once segmentation masks are available.

---

If you're joining the project, start by running the EDA notebooks and the preprocessing sanity check to verify the local data layout. Contributions and questions are welcome — please open an issue or a PR with proposed changes.
```// filepath: /Users/jonatanbuga/Desktop/MRI-Project/README.md
# MRI Segmentation — Mouse Brain Longitudinal Study
